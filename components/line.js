...
class line extends D3Component {
    ...
    initialize(node, props) {
        const svg = this.svg = d3.select(node).append('div').attr("class", "chart-wrapper").attr("id", "chart-line1");
        ...
        function makeLineChart(dataset, xName, yObjs, axisLables) {
            ...
            chartObj.xScale = d3.scaleLinear().range([0, chartObj.width]).domain([1971, 2018]); //< Can be overridden in definition

            // Get the max of every yFunct
            chartObj.max = function (fn) {
                return d3.max(chartObj.data, fn);
            };
            chartObj.yScale = d3.scaleLinear().range([chartObj.height, 0]).domain([0, 48510]);
